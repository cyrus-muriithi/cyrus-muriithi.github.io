...
pred <- Main.df %>% 
  ...
  mutate(MY = paste(lubridate::year(Dates),paste0("0",lubridate::month(Dates)), sep = " ")) %>% 
  group_by(MY) %>% 
  summarise(Confirmed = max(Confirmed))

#start.ts = as.Date( min(pred$Dates))
#end.ts = lubridate::decimal_date(max(pred$Dates))

ts.df <- as.data.frame(pred[,"Confirmed"]) 

tstime.ts <- ts(ts.df, start = c(2020,01), frequency = 12)

p<- forecast(tstime.ts, level = 95, h=2)
hchart(p) %>% 
  hc_add_theme(hc_theme_darkunica())
# timetk::tk_tbl(p)
```
