...
comp_trend<- data.frame(Main.df %>% 
                          ...
                          dplyr::select(id,Country,AfricaCheck,Dates,Confirmed,Deaths,Recovered,Active,NewConfirmed,NewRecoveries,NewDeaths) %>% 
                          tidyr::gather(Category,Cases, Confirmed:NewDeaths) %>% 
                          ...
                          mutate(dindex = 1))
NC<- comp_trend%>% filter(Country=="Kenya" & Category=="NewConfirmed") %>% select(Dates, Cases)
...
forecast <- forecast %>% mutate(id = seq(seq_len(n()))) %>% 
  filter(id<=160)
...
bins <- c(0,50, 100, 500, 1000, 5000,10000,15000,20000,25000, Inf)
