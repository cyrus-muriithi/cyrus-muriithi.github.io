...
pred <- Main.df %>% 
  ...
  summarise(Confirmed = round(max(Confirmed, na.rm = T),0),
    ...
    mutate(yearday = lubridate::yday(Dates)) %>%
  select(Dates,yearday,Confirmed)#%>% 


myts <- ts(rnorm(length(pred$Dates)),     # random data
           start = c(2020, as.numeric(format(pred$Dates[1], "%j"))),
           frequency = 365)

hw <- HoltWinters(myts)
hchart(forecast(hw,h=2))
```
...
bins <- c(0, 10, 20, 50, 100, 200, 500,750, 1000, 1200, 1500, 2000, 2500, 3000, Inf)
