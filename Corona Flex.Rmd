...
hc <- hchart(Jan, type = "line", hcaes(x = Dates, y = Totals, group = Variables)) %>% 
  ...
  hc_subtitle(text = paste(max(Jan$Totals, na.rm = T),"Cases", sep = " ")) %>% 
  hc_add_theme(hc_theme_monokai())
...
hc <- hchart(smoothtrend, type = "line", hcaes(x = Dates, y = TotalCases, group = Categories)) %>% 
  ...
  hc_subtitle(text = paste(max(smoothtrend$TotalCases, na.rm = T),"Cases", sep = " ")) %>% 
  hc_add_theme(hc_theme_darkunica()) %>% 
  hc_colors(cols)
...
pred <- Main.df %>% 
  filter(AfricaCheck==1 & Country=="Kenya") %>% 
  group_by(Dates) %>% 
  summarise(Confirmed = round(max(Confirmed, na.rm = T),0),
            Recovered = round(max(Recovered, na.rm = T),0),
            Deaths = round(max(Deaths, na.rm = T),0)) %>% select(Dates,Confirmed)


start.ts = min(pred$Dates)
end.ts = lubridate::decimal_date(max(pred$Dates))

# ts.df <- pred[,"Confirmed"]
# tstime.ts <- as.ts(ts(ts.df, start = start.ts, frequency = 7))

ts.df <- as.ts(timeSeries::substituteNA(as.ts(with(pred, zooreg(Confirmed, order.by = Dates, 
                       frequency = 7))), type = "zeros"))
plot(ts.df)
# library("xts")
# pred.xts <- xts(pred$Confirmed,order.by = pred$Dates)
p<- forecast(auto.arima(ts.df, stepwise = F, approximation = F), level = 95)
