...
hc <- hchart(Jan, type = "line", hcaes(x = Dates, y = Totals, group = Variables)) %>% 
  ...
  hc_subtitle(text = paste(format(max(Jan$Totals, na.rm = T),big.mark = ","),"Cases", sep = " ")) %>% 
  hc_add_theme(hc_theme_monokai())
...
tts <- format(as.numeric((Jan %>% group_by(Variables) %>% summarise(tt = max(Totals, na.rm = T))) %>% summarise(tt=sum(tt))),big.mark = ",")
...
hc <- hchart(Jan, type = "line", hcaes(x = Dates, y = Totals, group = Variables)) %>% 
  ...
  hc_chart(type = "Number of Cases", options3d = list(enabled = TRUE, beta = 0, alpha = 0)) %>% 
  hc_title(text = "January 2020 Trends") %>% 
  hc_subtitle(text = paste(tts,"Cases", sep = " ")) %>% 
  hc_add_theme(hc_theme_darkunica())
...
hc <- hchart(Feb, type = "line", hcaes(x = Dates, y = Totals, group = Variables)) %>% 
  hc_xAxis(title = list(text = "Months")) %>% 
  hc_yAxis(title = list(text = "Number of Cases")) %>% 
  hc_chart(type = "Number of Cases", options3d = list(enabled = TRUE, beta = 0, alpha = 0)) %>% 
  hc_title(text = "February 2020 Trends") %>% 
  hc_subtitle(text = paste(format(max(Feb$Totals, na.rm = T),big.mark = ","),"Cases", sep = " ")) %>% 
  hc_add_theme(hc_theme_monokai())
...
tts <- format(as.numeric((Feb %>% group_by(Variables) %>% summarise(tt = max(Totals, na.rm = T))) %>% summarise(tt=sum(tt))),big.mark = ",")
...
hc <- hchart(March, type = "line", hcaes(x = Dates, y = Totals, group = Variables)) %>% 
  hc_xAxis(title = list(text = "Months")) %>% 
  hc_yAxis(title = list(text = "Number of Cases")) %>% 
  hc_chart(type = "Number of Cases", options3d = list(enabled = TRUE, beta = 0, alpha = 0)) %>% 
  hc_title(text = "March 2020 Trends") %>% 
  hc_subtitle(text = paste(format(max(March$Totals, na.rm = T),big.mark = ","),"Cases", sep = " ")) %>% 
  hc_add_theme(hc_theme_monokai())
...
tts <- format(as.numeric((March %>% group_by(Variables) %>% summarise(tt = max(Totals, na.rm = T))) %>% summarise(tt=sum(tt))),big.mark = ",")
...
April - Trends {data-navmenu="Global - Trends" data-icon="fa-signal"}
=======================================================================
...
hc <- hchart(April, type = "line", hcaes(x = Dates, y = Totals, group = Variables)) %>% 
  hc_xAxis(title = list(text = "Months")) %>% 
  hc_yAxis(title = list(text = "Number of Cases")) %>% 
  hc_chart(type = "Number of Cases", options3d = list(enabled = TRUE, beta = 0, alpha = 0)) %>% 
  hc_title(text = "April 2020 Trends") %>% 
  hc_subtitle(text = paste(format(max(April$Totals, na.rm = T),big.mark = ","),"Cases", sep = " ")) %>% 
  hc_add_theme(hc_theme_monokai())

hc
```

### Death vs Recovery {data-navmenu="April"}
```{r}
April <- Overall %>% 
  filter(Month==4) %>% 
  tidyr::gather(Variables, Values, Confirmed:Recovered) %>% 
...
tts <- format(as.numeric((April %>% group_by(Variables) %>% summarise(tt = max(Totals, na.rm = T))) %>% summarise(tt=sum(tt))),big.mark = ",")
...
May - Trends {data-navmenu="Global - Trends" data-icon="fa-signal"}
=======================================================================
...
hc <- hchart(May, type = "line", hcaes(x = Dates, y = Totals, group = Variables)) %>% 
  hc_xAxis(title = list(text = "Months")) %>% 
  hc_yAxis(title = list(text = "Number of Cases")) %>% 
  hc_chart(type = "Number of Cases", options3d = list(enabled = TRUE, beta = 0, alpha = 0)) %>% 
  hc_title(text = "May 2020 Trends") %>% 
  hc_subtitle(text = paste(format(max(May$Totals, na.rm = T),big.mark = ","),"Cases", sep = " ")) %>% 
  hc_add_theme(hc_theme_monokai())

hc
```

### Death vs Recovery {data-navmenu="May"}
```{r}
May <- Overall %>% 
  filter(Month==5) %>% 
  tidyr::gather(Variables, Values, Confirmed:Recovered) %>% 
...
tts <- format(as.numeric((May %>% group_by(Variables) %>% summarise(tt = max(Totals, na.rm = T))) %>% summarise(tt=sum(tt))),big.mark = ",")
...
June - Trends {data-navmenu="Global - Trends" data-icon="fa-signal"}
=======================================================================
...
hc <- hchart(June, type = "line", hcaes(x = Dates, y = Totals, group = Variables)) %>% 
  hc_xAxis(title = list(text = "Months")) %>% 
  hc_yAxis(title = list(text = "Number of Cases")) %>% 
  hc_chart(type = "Number of Cases", options3d = list(enabled = TRUE, beta = 0, alpha = 0)) %>% 
  hc_title(text = "June 2020 Trends") %>% 
  hc_subtitle(text = paste(format(max(June$Totals, na.rm = T),big.mark = ","),"Cases", sep = " ")) %>% 
  hc_add_theme(hc_theme_monokai())

hc
```

### Death vs Recovery {data-navmenu="June"}
```{r}
June <- Overall %>% 
  filter(Month==6) %>% 
  tidyr::gather(Variables, Values, Confirmed:Recovered) %>% 
...
tts <- format(as.numeric((June %>% group_by(Variables) %>% summarise(tt = max(Totals, na.rm = T))) %>% summarise(tt=sum(tt))),big.mark = ",")
...
July - Trends {data-navmenu="Global - Trends" data-icon="fa-signal"}
=======================================================================
...
hc <- hchart(July, type = "line", hcaes(x = Dates, y = Totals, group = Variables)) %>% 
  hc_xAxis(title = list(text = "Months")) %>% 
  hc_yAxis(title = list(text = "Number of Cases")) %>% 
  hc_chart(type = "Number of Cases", options3d = list(enabled = TRUE, beta = 0, alpha = 0)) %>% 
  hc_title(text = "July 2020 Trends") %>% 
  hc_subtitle(text = paste(format(max(July$Totals, na.rm = T),big.mark = ","),"Cases", sep = " ")) %>% 
  hc_add_theme(hc_theme_monokai())

hc
```

### Death vs Recovery {data-navmenu="July"}
...
tts <- format(as.numeric((July %>% group_by(Variables) %>% summarise(tt = max(Totals, na.rm = T))) %>% summarise(tt=sum(tt))),big.mark = ",")
...
hc <- hchart(tmonths, type = "column", hcaes(x = Year_Month, y = TotalCases, group = Categories)) %>% 
  hc_xAxis(title = list(text = "Months")) %>% 
  hc_yAxis(title = list(text = "Number of Cases")) %>% 
