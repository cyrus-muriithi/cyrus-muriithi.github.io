...
InfRate <- Main.df %>% 
  ...
  mutate(NewInfectionRate = round(NewConfirmed / lag(NewConfirmed,default = first(NewConfirmed)) * 100,2),
         NewRecoveryRate = round(NewRecoveries / lag(NewRecoveries,default = first(NewRecoveries)) * 100,2) ,
         NewDeathRate = round(NewDeaths / lag(NewDeaths,default = first(NewDeaths)) * 100,2) ) %>% 
  #filter(NewInfectionRate>0 & !is.infinite(NewInfectionRate)) %>% 
  select(Dates, ends_with("Rate"))


InfRate[is.na(InfRate)]<- 0
InfRate[InfRate==Inf]<-0

InfRate<- InfRate %>% 
  tidyr::gather(Category, Rate, NewInfectionRate:NewDeathRate) #%>% 
  #mutate(Category = as.numeric(ifelse(Category=="NewInfectionRate",2,ifelse(Category=="NewRecoveryRate",3,1))))

ggplot(InfRate, aes(x=Dates, y=Rate, color=Category) )+
  stat_smooth(se = F)+theme_dark()
  #scale_color_gradientn(colours = rainbow(3))

cols<- 

library(highcharter)
hc <- hchart(InfRate, type = "line", hcaes(x = Dates, y = Rate, group = Category)) %>% 
  ...
  hc_subtitle(text = paste(format(max(InfRate$Rate, na.rm = T),big.mark = ","),"Cases", sep = " ")) %>% 
